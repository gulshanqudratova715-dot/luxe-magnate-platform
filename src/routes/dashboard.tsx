...
function Dashboard() {
  ...
  const [paymentStep, setPaymentStep] = useState<
    "form" | "authorizing" | "success" | "stripe_missing"
  >("form");
...
================================================`;
  ...
  return (
    <div className="mx-auto max-w-7xl px-6 sm:px-8 py-12 sm:py-16 space-y-8">
      ...
      {/* STRIPE PAYMENT GATEWAY DIALOG */}
      {checkoutPlan && (
        <div className="fixed inset-0 z-50 bg-background/80 backdrop-blur-sm grid place-items-center px-4 animate-fade-in">
          <div className="w-full max-w-md glass rounded-2xl border border-gold/20 p-6 sm:p-8 relative shadow-luxe">
            ...
            {paymentStep === "form" && (
              <div className="space-y-5">
                ...
                <div className="p-4 rounded-xl border border-gold/10 bg-muted/20 text-center space-y-1">
                  <span className="text-xs text-muted-foreground uppercase tracking-widest block">
                    Acquisition Tier Total
                  </span>
                  <span className="font-display text-2xl gold-text font-bold block">
                    {billingInterval === "annually"
                      ? checkoutPlan === "Elite"
                        ? "$79"
                        : checkoutPlan === "Magnate"
                          ? "$399"
                          : "$1,599"
                      : checkoutPlan === "Elite"
                        ? "$99"
                ...
                <button
                  ...
                  className="w-full gold-gradient text-primary-foreground py-3.5 rounded-md font-semibold text-sm hover-lift flex items-center justify-center gap-2 shadow-gold-glow"
                >
                  <CreditCard className="h-4 w-4" /> Pay with Secured Stripe Checkout
                </button>

                <div className="text-[10px] text-muted-foreground text-center">
                  Fully secure tokenized checkout processed natively by Stripe Inc.
                </div>
              </div>
            )}

            {paymentStep === "stripe_missing" && (
              <div className="space-y-5 py-2 text-center animate-fade-up">
                <div className="grid h-12 w-12 place-items-center rounded-full bg-red-950/40 border border-red-500/30 text-red-500 mx-auto mb-2 shadow-gold-glow">
                  <AlertTriangle className="h-6 w-6" />
                </div>
                <h3 className="font-display text-lg text-red-400">Stripe Integration Required</h3>
                <p className="text-xs text-muted-foreground max-w-xs mx-auto leading-relaxed">
                  This SaaS platform is pre-built with real production-ready **Stripe Subscription
                  Checkout**. However, no backend API keys are currently configured.
                </p>
                <div className="p-4 rounded-xl glass border border-gold/15 bg-muted/25 text-left text-[11px] text-muted-foreground space-y-2">
                  <p className="font-medium text-gold uppercase tracking-wider text-[10px]">
                    How to configure:
                  </p>
                  <p>1. Open the **Settings** menu in Google AI Studio (top-right corner).</p>
                  <p>
                    2. Add **
                    <code className="text-foreground bg-muted px-1.5 py-0.5 rounded font-mono">
                      STRIPE_SECRET_KEY
                    </code>
                    ** with your Stripe API secret key.
                  </p>
                  <p>
                    3. Refresh and try upgrading. Your secure Stripe Checkout portals will instantly
                    boot!
                  </p>
                </div>
                <button
                  onClick={() => setCheckoutPlan(null)}
                  className="w-full mt-2 border border-gold/30 text-gold py-2.5 rounded-md text-xs font-semibold hover:bg-gold/10 transition"
                >
                  Dismiss
                </button>
              </div>
