...
function Checkout() {
  ...
  const pay = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (!stripeRes.ok) {
        throw new Error("Failed to contact checkout service.");
      }

      const resData = await stripeRes.json();
      if (!resData.hasStripe) {
        // Rollback the created order since we are not using sandbox/simulators
  ...
  if (stripeConfigError) {
    return (
      <div className="mx-auto max-w-xl px-6 py-20 text-center animate-fade-up">
        <div className="grid h-14 w-14 mx-auto place-items-center rounded-full bg-red-950/40 border border-red-500/30 text-red-500 shadow-gold-glow mb-6">
          <AlertTriangle className="h-6 w-6" />
        </div>
        <h1 className="font-display text-3xl">Payment Gateway Configuration Required</h1>
        <p className="text-muted-foreground mt-4 text-sm leading-relaxed">
          This platform is fully equipped with real **Stripe Checkout**, but the owner's backend API
          keys have not been configured yet.
        </p>
        <div className="mt-8 p-6 rounded-2xl glass border border-gold/15 bg-muted/20 text-left space-y-3">
          <div className="text-xs uppercase tracking-widest text-gold font-semibold">
            How to Enable Live Payments:
          </div>
          <p className="text-xs text-muted-foreground leading-relaxed">
            1. Access the **Settings** menu in the top-right corner of Google AI Studio.
          </p>
          <p className="text-xs text-muted-foreground leading-relaxed">
            2. Add **
            ...
            ** as an environment variable under the platform secrets.
          </p>
          <p className="text-xs text-muted-foreground leading-relaxed">
            3. The Stripe Checkout session will automatically boot, offering secure and
            authenticated card payments for all high-end collections.
          </p>
        </div>
        <div className="mt-8 flex justify-center gap-4">
          <Link
            to="/marketplace"
            className="text-xs uppercase tracking-wider font-semibold border border-gold/25 text-gold hover:bg-gold/10 px-5 py-2.5 rounded transition"
          >
            Back to Marketplace
          </Link>
        </div>
